...
def place_boss_room(spawn_room, rooms, grid, visited, seed: int) -> Room | None:
    ...
    candidates = [c for c in raw_candidates if number_of_occupied_neighbors(c, visited) == 1]

    # Les trie de plus loin au plus proche de la salle de spawn et prend les 20% les plus éloignés
    sorted_by_far = sorted(candidates, key=lambda c: manhattan_distance(c, spawn_room.coord), reverse=True)
...
def split_budget(directions: list[str], total_budget: int, rng: random.Random) -> dict[str, int]:
    """Répartit un budget total entre les directions disponibles"""
    direction_keys = directions

    raw_weights = {d: rng.uniform(0.7, 1.3) for d in direction_keys} # Créé des poids aléatoires pour chaque direction
...
def maybe_drop_direction(available_directions: dict, rng: random.Random, drop_probability: float = 0.15) -> dict:
...
def number_of_occupied_neighbors(coord: tuple[int, int], visited: set[tuple[int, int]]) -> int:
    x, y = coord
    neighbors = [
        (x, y - 1),
        (x, y + 1),
    ...
    return sum(1 for neighbor in neighbors if neighbor in visited) # Somme des voisins occupés d'une case donnée
...
def find_available_neighbors(coord: tuple[int, int], rng: random.Random, grid: list[tuple[int, int]], visited: set[tuple[int, int]], step = 1,  path: list[tuple[int, int]] | None = None, previous_dir = None, continuity_bias: float = 0.5) -> list[tuple[int, int]] | None:
    ...
    directions = maybe_drop_direction(directions, rng, drop_probability=0.15)  # Retire une direction aléatoire avec une probabilité de 15%
    ...
    if previous_dir in dirs:
        weights = [
            continuity_bias if d == previous_dir else (1 - continuity_bias) / max(len(dirs) - 1, 1)
            for d in dirs
        ]
    else:
...
def generate_paths(grid: list[tuple[int, int]], rooms: list[Room], seed: int, max_rooms: int = 10, continuity_bias: float = 0.5, previous_dir = None, visited: set[tuple[int, int]] | None = None) -> list[Room]:
    ...
    available_directions = {direction: coord for direction, coord in spawn_neighbors.items() if coord in grid and coord not in visited}
    ...
    for direction, coord in available_directions.items():
        if direction_budgets[direction] > 0:
            ...
            path_length = direction_budgets[direction] - 1  # Déduire la salle déjà placée
            if path_length > 0:
                path = find_available_neighbors(coord, rng, grid, visited, step=path_length, previous_dir=previous_dir, continuity_bias=continuity_bias)
                if path:
                    for p in path:
...
if __name__ == "__main__":
    ...
    boss_room = place_boss_room(spawn_room, rooms, grid, {room.coord for room in rooms}, seed)
    if boss_room:
